...
final class TablePlaylist {
  ...
  static Future<bool> deletePlaylist(int playlistId) async {
    try {
      ...
      await db.transaction((txn) async {
        await txn
            .delete(TablePlaylistSong.name, where: '${TablePlaylistSong.playlistIdCol} = ?', whereArgs: [playlistId]);
  ...
  static Future<List<Playlist>> playlistsWithSongs(List<Song> songs) async {
    String sql =
        "SELECT ${TablePlaylist.name}.${TablePlaylist.idCol}, ${TablePlaylist.name}.${TablePlaylist.titleCol} FROM ${TablePlaylistSong.name}"
