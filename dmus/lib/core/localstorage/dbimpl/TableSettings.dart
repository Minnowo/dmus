...
final class TableSettings {
  ...
  static Future<Map<String, String?>> selectAll() async {
    ...
    return Map.fromEntries(r.map((map) => MapEntry(map[keyCol] as String, map[valueCol] as String?)));
  }
  ...
  static Future<void> save(Map<String, String?> settings) async {
    ...
    settings.entries.map((e) => {keyCol: e.key, valueCol: e.value}).forEach((d) => batch.insert(name, d));
