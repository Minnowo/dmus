...
final class ImportController {
  ...
  static Future<void> checkWatchFolders() async {
    ...
    MessagePublisher.publishSnackbar(
        SnackBarData(text: "${S.current.checkingDirectories1} ${watchDirs.length} ${S.current.checkingDirectories2}"));
  ...
  static Future<void> reimportAll() async {
    ...
    for (final i in results) {
      ...
      if (!await filePath.exists()) {
        MessagePublisher.publishSnackbar(
            SnackBarData(text: "${S.current.songPathDoesNotExist1} $path ${S.current.songPathDoesNotExist2}"));
  ...
  static Future<void> importSong(File path) async {
    ...
    if (songId == null) {
      ...
      if (!_silencePubs) {
        MessagePublisher.publishSomethingWentWrong(S.current.cannotImportSongDoesNotExist);
      }
      return;
    }
    ...
    if (s == null) {
      ...
      if (!_silencePubs) {
        MessagePublisher.publishSomethingWentWrong(S.current.dbError);
      }
      return;
    }
  ...
  static Future<void> importSongFromDirectory(Directory dir, bool isRecursive, bool importOnlyIfNotInDb) async {
    ...
    try {
      ...
      if (!_silencePubs) {
        MessagePublisher.publishSomethingWentWrong(S.current.noPermissionInDirectory);
      }
